...
* siapkan VPS di DigitalOcean

        tugboat create vps-jenkins
...
Kita akan disajikan pertanyaan apakah ingin menyimpan setting firewall yang aktif saat ini, mirip seperti pada waktu baru menginstall `iptables-persistent` tadi. Jawab `Yes` untuk kedua pertanyaan. Hasilnya bisa dipastikan dengan melihat isi `/etc/iptables/rules.v4`. Kira-kira seperti ini isinya

```
# Generated by iptables-save v1.6.0 on Sun Jan 22 14:34:46 2017
*filter
:INPUT ACCEPT [33282:100677729]
:FORWARD ACCEPT [0:0]
:OUTPUT ACCEPT [25130:11265254]
-A FORWARD -d 127.0.0.1/32 -i eth0 -p tcp -m state --state NEW -m tcp --dport 8443 -j ACCEPT
COMMIT
# Completed on Sun Jan 22 14:34:46 2017
# Generated by iptables-save v1.6.0 on Sun Jan 22 14:34:46 2017
*nat
:PREROUTING ACCEPT [26:1186]
:INPUT ACCEPT [161:9014]
:OUTPUT ACCEPT [3:2951]
:POSTROUTING ACCEPT [3:2951]
-A PREROUTING -i eth0 -p tcp -m tcp --dport 443 -j DNAT --to-destination 127.0.0.1:8443
-A OUTPUT -d 127.0.0.1/32 -p tcp -m tcp --dport 443 -j DNAT --to-destination 127.0.0.1:8443
COMMIT
# Completed on Sun Jan 22 14:34:46 2017
```
